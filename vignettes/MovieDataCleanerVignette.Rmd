...
<h3> Load package and dataset <h3>
...
Team was able to achieve the outcome with proposed Aim in given timeline using Knn functionality for imputing missing data, reducing errors with nulls also improving the efficiency with less missing values to the user in the end and also performed NMF functionality for the better dimension reduction approach and successfully conducted clustering techniques on that data.    

## Significance

This project would help to serve as a template for how to deal with large datasets with lots of missing data using k nearest neighbors and or machine learning algorithms. Missing data can be found in almost any area of interest, not just movies, therefore dealing with missing data is a problem that one is likely to encounter

## Real Life Example </h2> 

Movielens is a recommender-system that recommends movies based on users film preferences. In this package vignette, we used Movielens dataset to implement both aims and were successful. Therefore, this proves that the package works for any dataset in real life.
